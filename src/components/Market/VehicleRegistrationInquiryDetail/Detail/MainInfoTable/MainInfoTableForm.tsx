        ...
        <MainForSaleInfoTable>
          <tbody>
            <tr>
              ...
              <TableContent>{detailItem?.brand}</TableContent>
            </tr>
            <tr>
              {/* 모델 */}
              <TableHeader>모델</TableHeader>
              <TableContent>{detailItem?.model}</TableContent>
              {/* 변속기 */}
              <TableHeader>변속기</TableHeader>
              <TableContent>{detailItem?.transmissionType}</TableContent>
              {/* 연식 */}
              <TableHeader>연식</TableHeader>
              <TableContent>{detailItem?.regDate}</TableContent>
              {/* 형식연도 */}
              <TableHeader>형식연도</TableHeader>
              <TableContent>{detailItem?.year}</TableContent>
            </tr>
            <tr>
              {/* 연료 */}
              <TableHeader>연료</TableHeader>
              <TableContent>{detailItem?.fuel}</TableContent>
              {/* 배기량 */}
              <TableHeader>배기량</TableHeader>
              <TableContent>
                {detailItem?.cc.replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",")}
                cc
              </TableContent>
              {/* 주행거리 */}
              ...
              <TableContent>{detailItem?.color}</TableContent>
            </tr>
            <tr>
              {/* 사고 여부 */}
              ...
              <TableContent>{detailItem?.sellType}</TableContent>
              {/* 판매가격 */}
              <TableHeader>판매가격</TableHeader>
              <TableContent>
                {detailItem?.price.replace(
                  /\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g,
                  ","
                )}
              </TableContent>
