...
const ForSaleListForm = () => {
  ...
  const checkedItemHanlder = ( index : number, isChecked : boolean ) => {
    if( isChecked ){
      checkedItems.add( index );
      setCheckedItems( checkedItems );
    }else if( !isChecked && checkedItems.has( index ) ){
      checkedItems.delete( index );
      setCheckedItems( checkedItems );
    }
  };

  const checkHanlder = ( event : React.ChangeEvent<HTMLInputElement>, index : number ) => {
    setIsChecked( !isChecked );
    checkedItemHanlder( index, event.target.checked );
  }
        ...
        <TableWrapper>
          <MarketTable>
            ...
            { Array.from( {length:20}, ( v, i ) => {
              return (
                <MarketTableBody key={`uuid`+i}>
                  <tr>
                    <MarketTableBodyRowSpan index={0}>
                      <MarketCheckBoxWrapper>
                        <MarketInputCheckBox id={"body_check" + i} onChange={ event => checkHanlder( event, i ) }/>
