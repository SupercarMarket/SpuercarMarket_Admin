...
const MagazineListTableBodyForm = ({
  offset,
  postsPerPage,
  totalContentsCount,
  hideButtonOnClickHandler,
  unhideButtonOnClickHandler,
  deleteButtonOnClickHandler,
}: MagazineListTableProps) => {
  const inputCheckTypeRef = useRef<HTMLInputElement>(null);
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const { list, checkList } = useAppSelector(
    (state) => state.MagazineListSlice
  );
        ...
        return (
          <React.Fragment key={magazine.id}>
            <tr id={magazine.id.toString()} onClick={tableRowOnClickHandler}>
              ...
              <MagazineListTableBodyRowSpan>
                {String(magazine.id.toString()).padStart(7, "0")}
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                {magazine.title}
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                {magazine.createdDate}
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                {magazine.scrapCount}개
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                {magazine.cmtCount}개
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                {magazine.viewCount}개
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                <MagazineListTableBodyButton
                  onClick={() => magazineModifyOnClickHandler(magazine.id)}
                >
                  수정하기
                </MagazineListTableBodyButton>
              </MagazineListTableBodyRowSpan>
              <MagazineListTableBodyRowSpan>
                {magazine.hidden === true ? (
                  <MagazineListTableBodyButton
                    id={magazine.id.toString()}
                    onClick={() => unhideButtonOnClickHandler(magazine.id)}
                  >
                    숨기기 취소
                  </MagazineListTableBodyButton>
                ) : (
                  <MagazineListTableBodyButton
                    id={magazine.id.toString()}
                    onClick={() => hideButtonOnClickHandler([magazine.id])}
                  >
                    숨기기
                  </MagazineListTableBodyButton>
                )}
              ...
              <MagazineListTableBodyRowSpan>
                <MagazineListTableBodyButton
                  onClick={() => deleteButtonOnClickHandler(magazine.id)}
                >
                  삭제하기
                </MagazineListTableBodyButton>
              </MagazineListTableBodyRowSpan>
