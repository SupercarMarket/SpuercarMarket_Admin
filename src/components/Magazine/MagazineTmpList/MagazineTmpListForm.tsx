...
import { getMagazineTmpList, MagazineTmpAction } from "redux/modules/MagazineTmpSlice";
...
function MagazineTmpListForm() {
    ...
    const { isLoading, currentPage, totalCount } = useAppSelector((state) => state.MagazineTmpSlice);
    ...
    return (
        <Wrapper>
            <MagazineTmpHeader deleteMagazineTmpButtonOnClickHandler={deleteMagazineOnClickHandler} />
            {!isLoading ? (
                <>
                    <MagazineTmpListTableForm offset={offset} postsPerPage={postsPerPage} totalContentsCount={totalCount} deleteButtonOnClickHandler={deleteMagazineOnClickHandler} />
                </>
            ) : (
                <div>조회중입니다.</div>
            )}
