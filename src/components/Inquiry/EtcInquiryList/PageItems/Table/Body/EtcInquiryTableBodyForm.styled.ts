...
export const EtcInquiryTableBody = styled.tbody`
    height : 80px;
...
export const EtcInquiryTableBodyRowSpan = styled.td`
    text-align: center;
    padding : 19px 16px;
...
export const EtcInquiryTableBodyClamp = styled.div`
  overflow: hidden;
	display: -webkit-box;
...
export const EtcInquiryTableBodyNoSpan = styled.td.attrs({ })`
    text-align:center;
    vertical-align:middle;
    border : 1px solid ${({theme}) => theme.colors.greyScale_3};
`;

export const EtcInquiryTableBodyButton = styled(Button)<{ progress: number; }>`
    background-color: ${({progress}) => progress == 0 ? ({theme}) => theme.colors.greyScale_1 : ({theme}) => theme.colors.greyScale_3};
    color : ${({theme}) => theme.colors.greyScale_6};
...
export const EtcInquiryCheckBoxWrapper = styled.div`
...
export const EtcInquiryInputCheckBox = styled.input.attrs({ type : "checkbox" } )`
  display: none;
  /* + 연산 : 바로 옆 태그를 참조 */
  /* 체크 된 상태 CSS */ 
  &+label{
...
export const EtcInquiryLabelCheckBox = styled.label``;