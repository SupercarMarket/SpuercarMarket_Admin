...
} from "redux/modules/DealerInquirySlice";
...
function DealerInquiryListForm() {
  ...
  const { isLoading, filter, keyword, currentPage, totalCount, totalPages } =
    useAppSelector((state) => state.DealerInquirySlice);
    ...
    dealerAcceptHandler(userSeq)
      .then((response) => {
        if (response?.status === 200) {
          ...
          dispatch(
            DealerInquiryAction.setDealerInquiryListCount({
    ...
    <Wrapper>
      ...
      {!isLoading ? (
        <DealerInquiryTable
          ...
          registerDealerHandler={registerDealerHandler}
        />
      ) : (
        <div>조회 중입니다.</div>
      )}
