...
} from "./MemberTableBodyForm.styled";
        ...
        return (
          <React.Fragment key={user.userSeq}>
            <tr>
              ...
              <MemberTableBodyRowSpan rowSpan={2}>
                {String(user.userSeq.toString()).padStart(7, "0")}
              </MemberTableBodyRowSpan>
              <MemberTableBodyRowSpan colSpan={2}>
                {user.userId}
              </MemberTableBodyRowSpan>
                    ...
                    {user.isDealer === true ? (
                      <>
                        ...
                        <MemberTableBodyButton
                          id={user.userSeq.toString()}
                          onClick={() =>
                            memberDetailOnClickHandler(user.userSeq)
                          }
                        >
                          정보
                        </MemberTableBodyButton>
              ...
              <MemberTableBodyRowSpan rowSpan={2}>
                {user.isBanned ? (
                  <MemberTableBodyButton
                    onClick={() => unbanMemberHandler(user.userSeq)}
                  >
                    차단 풀기
                  </MemberTableBodyButton>
                ) : (
                  <MemberTableBodyButton
                    onClick={() => banMemberHandler(user.userSeq)}
                  >
                    회원 차단
                  </MemberTableBodyButton>
                )}
