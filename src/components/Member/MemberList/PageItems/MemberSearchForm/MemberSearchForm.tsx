...
import {
  MemberListDropDownMap,
  MemberListSwitchDropDownMap,
...
function MemberSearchForm() {
  ...
  const {
    ...
    level,
    levelAllChecked,
  } = useAppSelector((state) => state.MemberSlice);
  ...
  useEffect(() => {
    ...
    if (searchData.filter && DropDownTitleRef.current) {
      DropDownTitleRef.current.textContent =
        MemberListSwitchDropDownMap[searchData.filter as string];
    }
  }, []);
  ...
  // Role 선택
  const roleRadioBtnClickHandler = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  // Level 선택
  const levelAllCheckedClickHandler = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
  ...
  const levelCheckBoxClickHandler = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    ...
    <SearchBoxWrapper>
      <SearchBox>
        <div className="element">
          ...
          <div className="content">
            <DropDownForm
              ...
              LiOnClick={(event) => DropDownOnClickHandler(event)}
        ...
        <div className="element">
          ...
          <div className="content">
            ...
            >
              <input
                type="checkbox"
                onChange={allDateOnClickHandler}
                checked={searchData.allDate}
              />
              <span>전체</span>
            </div>
            <DateRangePickerForm
              ...
              {roleOptions.map((option) => (
                <RadioBtnLabel key={option.value}>
                  <input
                    type="radio"
                    value={option.value}
                    checked={option.value === searchData.role}
                    onChange={roleRadioBtnClickHandler}
                  />
          ...
          <div className="content">
            <CheckBoxWrapper>
              <CheckBoxLabel>
                <input
                  type="checkbox"
                  checked={searchData.levelAllChecked}
                  onChange={levelAllCheckedClickHandler}
                />
                <span>전체</span>
              </CheckBoxLabel>
              {levelOptions.map((option) => (
                <CheckBoxLabel key={option.value}>
                  <input
                    type="checkbox"
                    value={option.value}
                    checked={searchData.level.includes(option.value) === true}
                    onChange={levelCheckBoxClickHandler}
                  />
                  <span>{option.name}</span>
                </CheckBoxLabel>
              ))}
        ...
        <div className="element" style={{ justifyContent: "center" }}>
          <Button
            className="brown"
            onClick={SearchBarInputClickHandler}
