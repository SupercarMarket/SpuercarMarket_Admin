...
// 판매차량 등록 문의 리스트 조회
export const getForSaleInquiryList = createAsyncThunk('GET/getForSaleInquiryList', async( data : getForSaleListParamsType, thunkApi ) => {
    try{
        const response = await getForSaleListHandler( data.filter, data.keyword, data.page );
        return response;
    }catch( error ){
...
export const getForSaleDetailItem = createAsyncThunk(
  ...
  async ( params : getDetailParamsType, thunkApi) => {
    try {
        const response = await getDetailForSaleInquiryItemHandler( params.category, params.brdSeq );
        return response;
    } catch (error) {
        return thunkApi.rejectWithValue( error );
    }
